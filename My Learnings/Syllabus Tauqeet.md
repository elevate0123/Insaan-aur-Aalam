...
```mermaid
graph TD
    A[Earth's Rotation relative to Sun] --> B(Tool-e-Balad / Longitude)
    B --> C{Astronomical Formulas}
    C --> D[Baldi Waqt / Local Mean Time]
    
    E[Government Time Zone] --> F(Standard Time Offset e.g., GMT +5)
    
    B --> G((Divide by 15))
    F --> H{Subtract}
    G --> H
    
    H --> I[Tadeel-e-Murawwaj]
    
    D --> J{Add Tadeel-e-Murawwaj}
    I --> J
    
    J --> K[Meyari Waqt / Clock Time]
    
    style D fill:#f9d0c4,stroke:#333,stroke-width:2px
    style I fill:#fff2cc,stroke:#333,stroke-width:2px
    style K fill:#d5e8d4,stroke:#333,stroke-width:4px
```
...
```mermaid
graph TD
    A[Earth's 23.5° Axial Tilt] --> B(Mail-e-Shams / Declination)
    C[Earth's Elliptical Orbit] --> D(Tadeel-e-Ayyam / Equation of Time)
    
    B --> E{Impacts}
    E --> F[Shadow Lengths for Asr]
    E --> G[Sunrise/Sunset Trajectories]
    
    D --> H{Applied to 12:00:00 LMT}
    H --> I[Baldi Zawal / Local Noon]
    
    I --> J{Master Table / Jidawal}
    B --> J
    
    J --> K((Ready for Prayer Calculations))
    
    style B fill:#dae8fc,stroke:#333,stroke-width:2px
    style I fill:#d5e8d4,stroke:#333,stroke-width:2px
    style K fill:#ffe6cc,stroke:#333,stroke-width:4px
```
...
```mermaid
graph TD
    A(Sun rises in East) -->|Symmetrical Time/Distance| B{Meridian Transit}
    C(Sun sets in West) -->|Symmetrical Time/Distance| B
    
    B --> D[Nisf-un-Nahar Haqiqi]
    D --> E[Shadows at absolute minimum: Saya-e-Asli]
    D --> F[Makrooh / Prohibited Prayer Time]
    
    F -->|Sun visibly crosses axis| G[Zawal]
    G --> H[Zuhr Window Begins]

    style D fill:#f8cecc,stroke:#b85450,stroke-width:2px
    style E fill:#fff2cc,stroke:#d6b656,stroke-width:2px
    style H fill:#d5e8d4,stroke:#82b366,stroke-width:3px
```
...
```mermaid
graph TD
    A[Observer at Sea Level] --> B(Rational Horizon: 90°)
    
    B --> C{Atmospheric Refraction}
    C -->|Bends light up| D(+ 34')
    
    B --> E{Solar Semi-Diameter}
    E -->|Upper limb required| F(+ 16')
    
    D --> G[Standard Shari'i Zenith Distance: 90°50']
    F --> G
    
    H[Observer at Elevation e.g., Skyscraper/Plane] --> I{Earth's Curvature}
    I -->|Horizon Dips| J(+ Dip Formula)
    
    G --> K{Master Hour Angle Formula}
    J --> K
    
    K --> L[Exact Temporal Distance from Zawal]
    L --> M[Subtract for Sunrise]
    L --> N[Add for Sunset]
...
```mermaid
graph TD
    Z[Zenith: 0°] -->|Hour Angle| A[Meridian Transit / Zawal]
    
    A -->|Descends| B(Geometric Horizon: 90°)
    B -->|Refraction & Semi-Diameter| C[Visible Sunset / Maghrib: 90°50']
    
    C -->|Twilight Begins| D(Civil Twilight: 96°)
    D --> E(Nautical Twilight: 102°)
    E --> F[Astronomical Twilight Ends / Isha: 108°]
    
    F -->|Total Darkness| G[Midnight / Nadir]
    
    G -->|Sun Ascends| H[Astronomical Twilight Begins / Fajr: 108°]
    H --> I[Visible Sunrise / Tulu: 90°50']
    
    style C fill:#f8cecc,stroke:#b85450,stroke-width:2px
    style F fill:#1ba1e2,stroke:#006eaf,stroke-width:2px,color:#ffffff
...
```mermaid
graph TD
    A[Vertical Object: Height = 1] --> B(Sun at Zawal)
    B --> C{Saya-e-Asli / Base Shadow}
    C -->|Calculated via| D[Tan Arz - Mail]
    
    D --> E{Choose Fiqhi Standard}
    E -->|Shafi'i/Maliki/Hanbali| F[Misl-e-Awal: Add 1]
    E -->|Hanafi| G[Misl-e-Sani: Add 2]
    
    F --> H{Inverse Tangent: Tan^-1}
    G --> H
    
    H --> I[Asr Zenith Distance]
    
    I --> J{Master Hour Angle Formula}
    J --> K[Time from Zawal]
    K --> L[Add to Standard Zawal = Asr Time]

    style G fill:#fff2cc,stroke:#d6b656,stroke-width:3px
    style H fill:#dae8fc,stroke:#6c8ebf,stroke-width:2px
    style L fill:#d5e8d4,stroke:#82b366,stroke-width:3px
```
...
```mermaid
graph TD
    A[City Longitude] --> B{Compare to Kaaba 39°50'E}
    B -->|> 39°50'| C[Base Axis: WEST]
    B -->|< 39°50'| D[Base Axis: EAST]
    
    E[Master Qibla Formula] --> F{Result Sign}
    F -->|Positive +| G[Deviation: NORTH]
    F -->|Negative -| H[Deviation: SOUTH]
    ...
    I --> J[Final Qibla Vector e.g., South of West]
    J --> K[Apply Magnetic Declination]
    K --> L[Physical Compass Bearing]

    style E fill:#dae8fc,stroke:#6c8ebf,stroke-width:2px
    style J fill:#fff2cc,stroke:#d6b656,stroke-width:3px
    style L fill:#d5e8d4,stroke:#82b366,stroke-width:3px
```
