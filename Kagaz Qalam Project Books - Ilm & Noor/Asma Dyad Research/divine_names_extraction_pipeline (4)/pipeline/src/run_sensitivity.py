...
def main():
    ...
    for threshold in SENSITIVITY_THRESHOLDS:
        ...
        pair_counts = Counter(
            # BUGFIX (found via user's first real run): must canonicalize to
            # an UNORDERED pair here. The raw dyad rows store name_1/name_2
            # by POSITION IN VERSE (whichever name appears first), which is
            # legitimate as per-row metadata (direction is tracked, per
            # Dossier §4.4) but means the SAME semantic pair can appear as
            # (A,B) in one verse and (B,A) in another. Counting on the
            # ordered tuple silently fragments every pair's frequency across
            # up to 2 buckets and inflates "unique pairs" — this is exactly
            # what happened in the first real run (Allāh-Al-Ḥaqq counted as
            # 2 separate "pairs": (1,52) and (52,1)).
            tuple(sorted((d["name_1_serial"], d["name_2_serial"])))
            for d in all_dyads
        )
