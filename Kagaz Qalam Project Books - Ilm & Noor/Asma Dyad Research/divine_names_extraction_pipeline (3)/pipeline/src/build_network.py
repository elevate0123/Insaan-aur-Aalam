...
def main():
    ...
    try:
        ...
        if GRAPH_GRAPHML_PATH.exists() and GRAPH_GRAPHML_PATH.stat().st_size == 0:
            GRAPH_GRAPHML_PATH.unlink()
    ...
    try:
        ...
        if GRAPH_GEXF_PATH.exists() and GRAPH_GEXF_PATH.stat().st_size == 0:
            GRAPH_GEXF_PATH.unlink()
            print(f"  Removed the resulting 0-byte file rather than leaving a silent empty artifact.")
