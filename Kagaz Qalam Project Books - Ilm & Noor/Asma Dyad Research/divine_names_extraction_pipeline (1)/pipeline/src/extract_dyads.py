    ...
    for (tok_a, names_a), (tok_b, names_b) in combinations(matches, 2):
        ...
        for name_a in names_a:
            for name_b in names_b:
                ...
                ) or (tok_a.confidence == 0.0 or tok_b.confidence == 0.0) or (len(names_a) > 1 or len(names_b) > 1)
...
def detect_refrains(dyad_rows):
    """
    Flags (surah, name_1, name_2) triples occurring >= REFRAIN_MIN_OCCURRENCES
    ...
    for row in dyad_rows:
        key = (row["surah_number"], row["name_1_serial"], row["name_2_serial"])
...
def main():
    ...
    refrains = detect_refrains(all_dyads)
    ...
    write_csv(args.refrain_out, refrains, logger)
